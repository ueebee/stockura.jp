...
class ScheduledListedInfoApiTester:
    ...
    def __init__(self, base_url: str = "http://app:8000"):
        self.base_url = base_url
        self.api_base = f"{base_url}/api/v1"
        self.created_schedule_ids: List[str] = []  # クリーンアップ用

    def generate_cron_expression(
        self, seconds_from_now: int = 15
    ) -> Tuple[str, datetime]:
        """指定秒後の cron 式を生成

        Args:
            seconds_from_now: 現在時刻から何秒後に実行するか（デフォルト: 15秒）

        Returns:
            (cron_expression, execution_time) のタプル
        """
        target_time = datetime.now() + timedelta(seconds=seconds_from_now)

        # cron 式を生成（分 時 日 月 曜日）
        cron_expression = f"{target_time.minute} {target_time.hour} * * *"

        logger.info(
            f"cron 式生成: {cron_expression} (実行予定: {target_time.strftime('%Y-%m-%d %H:%M')})"
        )
    ...
    def create_schedule(
        ...
        logger.info(
            f"  データ: {json.dumps(request_data, ensure_ascii=False, indent=2)}"
        )
    ...
    def wait_for_execution(self, execution_time: datetime, buffer_seconds: int = 10):
        """指定時刻まで待機
    ...
    def get_execution_history(
        self, schedule_id: str, timeout: int = 300, interval: int = 5
    ) -> Optional[Dict]:
        """実行履歴を監視して取得
    ...
    def run_full_test(self, target_date: str = "2024-08-06", wait_seconds: int = 15):
        """完全なテストシナリオを実行

        Args:
            target_date: データ取得対象日付 (YYYY-MM-DD)
            wait_seconds: 実行までの待機時間（秒）
        """
        logger.info("=" * 80)
        logger.info("スケジュール作成テスト開始")
        logger.info(f"対象日付: {target_date}")
        logger.info(f"待機時間: {wait_seconds}秒後に実行")
        logger.info("=" * 80)
        ...
        try:
            ...
            # 2. cron 式生成
            cron_expression, execution_time = self.generate_cron_expression(
                seconds_from_now=wait_seconds
            )
            ...
            if result_data:
                ...
                if isinstance(result_data, dict):
                    ...
                    if "execution_time" in result_data:
                        logger.info(
                            f"    実行時間: {result_data['execution_time']:.2f}秒"
                        )

                    if "from_date" in result_data and "to_date" in result_data:
                        logger.info(
                            f"    対象期間: {result_data['from_date']} 〜 {result_data['to_date']}"
                        )
...
def main():
    ...
    try:
        tester.run_full_test(
            target_date=args.target_date, wait_seconds=args.wait_seconds
        )
        logger.info("\n テストが正常に完了しました ✅")
