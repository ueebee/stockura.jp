"""Listed info Celery task with async-to-sync wrapper."""
...
@celery_app.task(bind=True, base=FetchListedInfoTask, name="fetch_listed_info_task_wrapped")
def fetch_listed_info_task_wrapped(
    self,
    ...
    """
    Fetch listed info data from J-Quants API using async-to-sync wrapper.
    ...
    logger.info(
        f"Starting fetch_listed_info_task_wrapped - task_id: {task_id}, "
        f"schedule_id: {schedule_id}, period_type: {period_type}"
    )

    # Use asgiref to convert async to sync
    # This creates a new event loop for each invocation
    return async_to_sync(_fetch_listed_info_async)(
        task_id=task_id,
        log_id=log_id,
...
async def _fetch_listed_info_async(
    ...
    # IMPORTANT: Each async_to_sync call creates a new event loop
    # This means database connections should be created fresh
    async with get_async_session_context() as session:
        ...
        task_log = TaskExecutionLog(
            ...
            task_name="fetch_listed_info_task_wrapped",
            task_id=task_id,
